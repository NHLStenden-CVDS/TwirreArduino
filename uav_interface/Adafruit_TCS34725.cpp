...
Adafruit_TCS34725::Adafruit_TCS34725(char* name, tcs34725IntegrationTime_t it, tcs34725Gain_t gain) : Device(name, "This is a RCS34725 RGB Sensor") 
{
  ...
  _AddVariable("clear", &_clear);
  _AddVariable("temp", &_temp);
  _AddVariable("lux", &_lux);

  _AddVariable("integration", &_sintegration);
  _AddVariable("gain", &_sgain);
  
  _AddVariable("led", &_led);
...
/*                           PUBLIC FUNCTIONS                             */
/*========================================================================*/
void Adafruit_TCS34725::ValuesChanged()
{
  digitalWrite(LED_PIN, _led);

  if(_sintegration!=cintegration){
    switch(_sintegration){
      case 0:
...
void Adafruit_TCS34725::OnRequest()
{

  getRawData(&_red, &_green, &_blue, &_clear);
  _temp = calculateColorTemperature(_red,_green,_blue);
